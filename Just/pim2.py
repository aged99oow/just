...
class PyxelInputMethod(InputMethod):
    def __init__(self):
        ...
        self.input_meaning_clip = ''  # 入力文字列の意味（クリップボード用）
    ...
    def set_input_meaning(self):  # 入力文字列の意味
        self.input_meaning = ''
        self.input_meaning_clip = ''
        meanings = pwdic.KANA_DIC.get(self.fixed_txt)
        if meanings:
            note = pwdic.form_notation(self.fixed_txt,meanings[0])
            mean = meanings[1].rstrip('※＃')
            self.input_meaning = YELLOW+pwdic.form_txtbox(note,68,1,'','…')+'\n'+ORANGE+pwdic.form_txtbox(mean,68,2,'','…')
            self.input_meaning_clip = note+'\n'+mean+'\n'
    ...
    def any_key(self, input_txt, changed=False):
